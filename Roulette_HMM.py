...
def simulate_roulette_sequence(states, observations, start_prob, trans_prob, emit_prob, length=30, seed=None):
    if seed is not None:
        random.seed(seed)
        np.random.seed(seed)
    state_seq = []
...
if __name__ == "__main__":
    ...
    # Simulate a roulette sequence
    true_states, obs_seq = simulate_roulette_sequence(states, observations, start_prob, trans_prob, emit_prob, length=30, seed=42)
    print("Observed sequence:", obs_seq)
    print("True hidden states:", true_states)
    hmm = RouletteHMM(states, observations, start_prob, trans_prob, emit_prob)
